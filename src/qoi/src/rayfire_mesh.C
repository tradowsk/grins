...
namespace GRINS
{
  ...
  bool RayfireMesh::validate_edge(const libMesh::Point & start_point, const libMesh::Point & end_point, const libMesh::Elem * side_elem, const libMesh::Elem * neighbor)
  {
    ...
    const libMesh::Node * node0 = side_elem->get_node(0);
    const libMesh::Node * node1 = side_elem->get_node(1);

    unsigned int side = libMesh::invalid_uint;

    for (unsigned int s=0; s<neighbor->n_sides(); s++)
      {
        libMesh::UniquePtr<libMesh::Elem> side_elem = neighbor->build_side(s); 
    ...
    if ( side != libMesh::invalid_uint )
      {
        libMesh::UniquePtr<libMesh::Elem> edge_elem = neighbor->build_side(side);
        ...
        if ( end_point_on_edge && start_point_on_edge )
          {
            bool l_to_r = ( start_point.absolute_fuzzy_equals(*(edge_elem->get_node(0))) ) && ( end_point.absolute_fuzzy_equals(*(edge_elem->get_node(1))) );
            bool r_to_l = ( end_point.absolute_fuzzy_equals(*(edge_elem->get_node(0))) )   && ( start_point.absolute_fuzzy_equals(*(edge_elem->get_node(1))) );
    ...
    if (is_valid)
    ...
    else
      {
        for (unsigned int c=0; c<neighbor->n_children(); c++)
          {
            const libMesh::Elem * child = neighbor->child(c);
